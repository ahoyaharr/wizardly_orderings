...
def processInput(s, max_nodes):
    ...
    if (len(line2) != N):
        return "Number of wizards present doesn't match the number of wizards in line 1. Line 1 says there are {} wizards, but line 2 contains {} wizards".format(
            N, len(line2))
    ...
    if (len(node_set) != len(line2)):
        return "Wizards cannot appear multiple times in the perfect age ordering. There are {} total wizards, but only {} unique wizards in the perfect ordering.".format(
            len(line2), len(node_set))
    ...
    for i in range(num_constraints):
        ...
        if (wiz_a < wiz_mid < wiz_b) or (wiz_b < wiz_mid < wiz_a):
            return "In line {i}, you said {wizard_mid}'s age was NOT in between {wizard_a} and {wizard_b}'s, however, in your optimal ordering, {wizard_a} appeared at {a_order}, {wizard_b} appeared at {b_order}, {wizard_mid} appeared at {mid_order}".format(
                i=line_num, wizard_a=constraint[0], wizard_b=constraint[1], wizard_mid=constraint[2], a_order=wiz_a,
                b_order=wiz_b, mid_order=wiz_mid)

    # Checks that there are no lines after the number of constraints are exhausted
    if fin.readline().split() != []:
        return "You said that there were {} constraints, but there was more input in the lines below that".format(
            num_constraints)
